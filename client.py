    ...
    def _decode_lw004_pb_payload(self, payload_data, fport):
        ...
        try:
            ...
            # FPort 1: Event Message Payload
                ...
                try:
                    result["timestamp_utc"] = datetime.utcfromtimestamp(timestamp_unix).isoformat() + "Z"
                except ValueError:
...
class CombinedApp:
    ...
    def handle_aws_message(self, topic, message, decoded_payload):
        ...
        print(f"Callback: Message received on topic '{topic}'") # Debug print
        try:
            timestamp = datetime.now().isoformat()
            alert_topic = self.settings.get('alert_topic', DEFAULT_ALERT_TOPIC)

            # Determine beacon MAC and Room
            ...
            if "WirelessMetadata" in message and "LoRaWAN" in message["WirelessMetadata"]:
                ...
                if lorawan.get("Gateways") and isinstance(lorawan["Gateways"], list) and len(lorawan["Gateways"]) > 0:
                    rssi = lorawan["Gateways"][0].get("Rssi")

            # Prioritize info from decoded payload if available
            is_alert = False
            if decoded_payload:
                # Check for alarm conditions in payload
                aux_op = decoded_payload.get("auxiliary_operation", "")
                if "Alert Alarm" in aux_op or "SOS Alarm" in aux_op:
                     is_alert = True

                # Update beacon signal info if beacons are detected
                if "beacons" in decoded_payload and decoded_payload["beacons"]:
                    for detected in decoded_payload["beacons"]:
                        mac = detected.get("mac")
                        det_rssi = detected.get("rssi")
                        det_dist = detected.get("estimated_distance")

                        # Update DB for this specific detected beacon
                        self.db.update_beacon_signal(
                            ...
                            estimated_distance=det_dist
                        )
                        # If the primary device ID matches one of the detected beacons,
                        # use its info for the main log/alert event.
                        if mac == beacon_mac:
                            rssi = det_rssi # Use the specific beacon's RSSI

                else: # No beacons detected, update signal for the main device ID
                     self.db.update_beacon_signal(
                          beacon_mac, rssi, # Use gateway RSSI if available
                          battery_level=decoded_payload.get("battery_level"),
                          is_charging=decoded_payload.get("is_charging"),
                          device_mode=decoded_payload.get("device_mode"),
                          auxiliary_operation=decoded_payload.get("auxiliary_operation")
            ...
            # Check if the topic itself indicates an alert
            if not is_alert and alert_topic and topic.startswith(alert_topic.replace('#', '').replace('+', '')):
                 is_alert = True


            # Look up room number and description from mapping
            if beacon_mac:
                mapping_info = self.beacons_mapping.get(beacon_mac)
                if mapping_info:
                    room_number = mapping_info.get("room_number", "Mapping Error")
                    description = mapping_info.get("description", "")
                else:
                    room_number = "Unknown Beacon" # Not in mapping

            # Log activity in the database (consider logging all messages or just specific ones)
            log_details = f"Topic: {topic}, RSSI: {rssi}, Decoded: {json.dumps(decoded_payload)}"
            beacon_db_id = None
            if beacon_mac:
                 beacon_record = self.db.get_beacon_by_mac(beacon_mac)
                 if beacon_record:
                      beacon_db_id = beacon_record[0] # Get the ID

            self.db.log_activity("MQTT_MSG", log_details, beacon_id=beacon_db_id)


            # If it's an alert, add to history and show notification
            if is_alert:
                alert_data = {
                    "timestamp": timestamp,
                    "topic": topic,
            ...
            print(f"Error handling AWS message processing: {str(e)}")
    ...
    def refresh_history_display(self):
        ...
        self.history_text.tag_configure('room', font=("Segoe UI", 12, "bold"), foreground="#2980b9") # Blue for room
        self.history_text.tag_configure('mac', font=("Consolas", 10), foreground="#3f51b5")
        self.history_text.tag_configure('desc', font=("Segoe UI", 10, "italic"), foreground="#7f8c8d") # Grey italic for desc
        ...
        for i, alarm in enumerate(sorted_history):
            ...
            # Format timestamp
             ...
             # Format entry
             self.history_text.insert(tk.END, f"🚨 ALARM #{len(sorted_history) - i}\n", 'heading') # Number newest as #1
        ...
        if displayed_count == 0:
             ...
             # else: # Already handled by the initial check
             #     self.history_text.insert(tk.END, "Alarm history is empty.", "empty")


        # Update status bar
        total_alarms = len(sorted_history)
    ...
    # --- Beacon Mapping Handling (from client.py) ---
    def load_beacons_mapping(self):
        """Load beacon room mapping from the JSON file specified in settings"""
        # Use the path from SettingsManager if available, otherwise fallback to client.py's original logic path
        # This needs careful consideration: admin.py used DB, client.py used JSON.
        # Let's prioritize the database managed by admin functions.
        # We'll populate self.beacons_mapping from the DB for consistency.
        ...
        try:
             ...
             for beacon_row in all_db_beacons:
                  ...
                  desc = beacon_row[3]
                  if mac:
                       self.beacons_mapping[mac] = {"room_number": room, "description": desc or ""}
    ...
    def setup_admin_dashboard(self, parent_window):
        ...
        active_frame = ttk.LabelFrame(live_data_pane, text="Active Beacons")
        ...
        # --- Recent Events Panel ---
        events_frame = ttk.LabelFrame(live_data_pane, text="Recent Raw Events")
    ...
    def refresh_admin_dashboard_data(self):
         ...
         for beacon in beacons:
             ...
             if last_seen_iso:
                 try:
                     ...
                     time_diff_secs = (current_time - last_seen_dt).total_seconds()
         ...
         # --- Update Recent Events (Example: last 5 MQTT messages from DB log) ---
         ...
         for log_entry in recent_raw_logs:
              ...
              # Only show MQTT messages here for brevity, filter if needed
              if ev_type == "MQTT_MSG":
                   event_count += 1
                   try:
              ...
              if event_count >= 10: # Limit display to 10 events
                   break
    ...
    def show_beacon_details_admin(self, event):
        ...
        selected_mac = self.admin_active_tree.item(selected_items[0], 'values')[0]
        ...
        if beacon_data:
            ...
            for i, field in enumerate(field_names):
                 value = beacon_data[i]
                 display_value = value
                 if field == "Last Seen" and value:
                      try: display_value = datetime.fromisoformat(value).strftime("%Y-%m-%d %H:%M:%S")
                      except: pass
                 elif field == "Created" and value:
                      try: display_value = datetime.fromisoformat(value).strftime("%Y-%m-%d %H:%M:%S")
                      except: pass
            ...
            self.admin_details_text.insert(tk.END, "Could not retrieve details for selected beacon.")
    ...
    def sort_beacon_column(self, col, reverse):
         ...
         # Basic sort - needs improvement for numeric/date types
         try:
              # Attempt numeric sort for relevant columns
              if col in ['id', 'last_rssi']:
                   data.sort(key=lambda t: int(t[0]) if t[0] is not None and t[0].isdigit() else -1, reverse=reverse)
              # Attempt date sort for date columns
              elif col in ['last_seen', 'created_at']:
                    data.sort(key=lambda t: datetime.fromisoformat(self.db.get_beacon_by_id(self.admin_beacon_tree.item(t[1])['values'][0])[4 if col=='last_seen' else 11] or '1970-01-01T00:00:00') , reverse=reverse)
              else:
                   data.sort(reverse=reverse)
         except Exception as e:
              print(f"Sorting error for column {col}: {e}. Using basic string sort.")
              data.sort(reverse=reverse) # Fallback to string sort
    ...
    def refresh_beacons_admin(self):
        ...
        for beacon in beacons:
             ...
             last_seen_str = "Never"
             if beacon[4]:
                 try: last_seen_str = datetime.fromisoformat(beacon[4]).strftime("%Y-%m-%d %H:%M:%S")
                 except: last_seen_str = beacon[4] # Show raw if format error

             created_str = "N/A"
             if beacon[11]:
                 try: created_str = datetime.fromisoformat(beacon[11]).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def refresh_logs_admin(self):
        ...
        for log in logs:
             ...
             try:
                 ts_str = datetime.fromisoformat(ts_iso).strftime("%Y-%m-%d %H:%M:%S")
             except:
                 ts_str = ts_iso
    ...
    def add_beacon_dialog_admin(self, initial_mac=""):
        ...
        # Paste button
        def paste_mac():
             try: mac_var.set(dialog.clipboard_get())
    ...
    def copy_mac_from_active_admin(self, event=None):
        ...
        if selected:
            mac = self.admin_active_tree.item(selected[0], 'values')[0]
            ...
            # Log to admin recent events? Maybe too noisy.
            return mac
        return None
        ...
        for log in recent_logs:
            ...
            for mac in matches:
                 ...
                 if mac_upper not in registered_macs:
                      unknown_macs.add(mac_upper)
            # Check MAC from log entry itself if present
            log_mac = log[2]
            if log_mac and log_mac.upper() not in registered_macs:
                 unknown_macs.add(log_mac.upper())
